...
export default class extends Controller {
  ...
  navigate (event) {
    const isHidden = !this.hasInputTarget || this.inputTarget.classList.contains('hidden')
  ...
  down () {
    const element = this.getCellByPosition(1)?.closest('.spreadsheet--cell')
    element?.focus()
    this.simulateMouseEvent(element, 'mousedown')
  }

  up () {
    const element = this.getCellByPosition(-1)
  ...
  getCellByPosition (upDown) {
    ...
    const x = this.element.dataset.lastX || rect.left + center
    const target = document.elementFromPoint(x, middle + height * upDown)?.closest('.spreadsheet--cell')
